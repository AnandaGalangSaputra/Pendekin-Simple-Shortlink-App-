...
<template>
  <div class="min-h-screen grid grid-cols-1 md:grid-cols-2 items-center px-8 md:px-16 bg-white dark:bg-gray-900">
    <!-- Kiri -->
    <div>
      ...
      <!-- Input nama shortlink -->
      <div class="mt-4">
        <div class="flex rounded-2xl border border-gray-300 dark:border-gray-700 overflow-hidden shadow-sm">
          <span class="px-4 py-3 bg-blue-600 text-white font-semibold select-none">
            https://pendekin/
          </span>
          <input
            v-model="shortName"
      ...
      <transition name="fade">
        <div v-if="showAlert" class="alert alert-error mt-4 transition-opacity duration-500">
          <span>{{ errorMsg }}</span>
        </div>
      </transition>
      ...
      <div class="mt-8">
        <button
          @click="handleSubmit"
          :disabled="loading"
          class="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl px-8 py-4 transition w-full md:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
        >
    ...
    <div
      class="modal-box relative bg-base-100 shadow-xl border border-base-200"
    >
      <!-- Tombol Close -->
      <button
      ...
      <p class="text-sm text-base-content/70 mt-1">
        ...
        <!-- Link akan kedaluwarsa pada <strong>{{ res.data.data.expired_at }}</strong> -->
      </p>
      ...
      <div class="modal-action">
        ...
        <button class="btn" @click="copyLink">Salin</button>
      </div>
    </div>
  </div>
</template>
